...
def test_machine_queue_focus_prefers_occupied_card_over_next_pending(connection):
    ...
    assert db.release_card(pending_card_id, machine_id=2, machine_sequence=1).ok
    assert db.release_card(paused_card_id, machine_id=2, machine_sequence=5).ok
    connection.execute(
...
def test_terminal_material_field_update_blocks_stale_version(connection):
    ...
    assert db.release_card(card_id, machine_id=4, machine_sequence=1).ok
    loaded_version = db.fetch_terminal_card_detail(card_id)["version"]
